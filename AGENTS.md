...
## What this is
Single-package TypeScript repo: a scheduling/booking data layer on LibSQL/Turso —
availability engine, multi-host routing, atomic booking writes. Runtime code lives in
`src/` (Cloudflare Worker entry: `src/worker.ts`); one-off DB scripts in `scripts/`;
tests are colocated with their modules (`src/*.test.ts`). Config stays at the root
(`tsconfig.json`, `wrangler.toml`, `drizzle.config.ts`, `package.json`).
ESM (`"type": "module"`); `tsconfig.json` includes `src/**`, `scripts/**`, and `drizzle.config.ts`.

## Commands
- `npm test` — runs `tsx --test "src/**/*.test.ts"` (quoted glob, expanded by the
  Node test runner; a new `src/*.test.ts` is picked up automatically).
- Single file/test: `npx tsx --test src/availability-engine.test.ts`
  (add `--test-name-pattern "<title>"` for one test).
- `npm run typecheck` — `tsc --noEmit`; there is no build step.
- `npm run schema:apply` — executes `src/schema.sql` against a live LibSQL instance.
- `npm run libsql:start` — runs `sqld`, a global binary NOT installed via npm.

## Env gotchas
- No dotenv dependency; npm scripts never load `.env` (only `src/daily.ts` parses it itself).
...
- `LIBSQL_URL`/`TURSO_DATABASE_URL` must be `http(s)://` or `libsql://` —
  `scripts/apply-schema.ts` and `drizzle.config.ts` deliberately reject `file:`/`.db` URLs.
...
`src/schema.ts` (canonical Drizzle) and `src/schema.sql` (applied identically to tests via
`src/schema-sql.ts` and to prod via `scripts/apply-schema.ts`). `src/test-db.ts` no longer holds its own
DDL, and `src/libsql-instance.test.ts` derives expected tables from `src/schema.ts`. No migrations
...
- Redaction policy: no real addresses, phone numbers, or live hostnames in docs or code —
  ...
  (Code-side scrub of `src/notifications.ts` and the test fixture is pending as Item 0.)

## Architecture rules
- `src/availability-engine.ts` is pure domain code: depends only on the `AvailabilityRepository`
...
- Concurrency model: `host_occupancy_ticks` unique (host, minute-tick) is SQLite's stand-in
  ...
  contention test in `src/libsql-instance.test.ts`. The former `host_mutexes` table was a no-op
...
- `src/libsql-instance.test.ts` self-skips unless `LIBSQL_URL`/`TURSO_DATABASE_URL` points at a
  live instance — a green `npm test` does not mean those ran. Same for the live Daily.co
  tests (`src/daily.test.ts` + the `dailyTest` flow tests), which self-skip without
...
## Stubs
`src/notifications.ts` only logs — the AgentMail send is an unimplemented seam (content is
built for real; only delivery is stubbed). Don't treat these as real integrations.

## Daily.co rooms
`src/daily.ts` is a real integration (key from `DAILY_API_KEY`; all failures soft-fail and
are logged). Room policy lives in `resolveChosenLocation` (src/create-booking-handler.ts):
