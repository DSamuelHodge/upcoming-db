...
| Change | Why |
|---|---|
| New **Item 0** (env/PII hygiene) moved ahead of everything | Address/phone in `notifications.ts` and the live Turso hostname in `AGENTS.md` were already public on GitHub; a 20-minute fix should not wait behind multi-day schema work. |
...
| v1 Phase 6.1 (DST test suite) trimmed to a single delta test | Spring-forward gap-start (`availability-engine.test.ts:41`), gap-end (`:206`), fall-back fold first-occurrence (`:87`), and overnight-into-gap (`:190`) tests already exist. Only the "both ends nonexistent on the same local day" case is missing. |
...
2. **Docs PII exposure:** scrub real addresses/phones/hostnames across all current and historical documentation with `[REDACTED_*]` placeholders. Code fixtures (`create-booking-handler.test.ts:14-15`) are scrubbed in Item 0, not here.
...
1. **`notifications.ts`** — remove hardcoded `[REDACTED_ADDRESS]`, `MAPS_URL`, and `[REDACTED_PHONE]` (`:15, 22, 26, 41, 45`). Read from env (`BUSINESS_ADDRESS` / `BUSINESS_MAPS_URL` / `BUSINESS_PHONE`); if unset, emit empty + a neutral "contact us" line.
2. **`daily.ts`** — delete the `readFileSync(".env")` homemade parser (lines 1–17, usage at `:29`). Require `DAILY_API_KEY` from `process.env`; `createDailyRoom` keeps returning `null` when absent. Aligns with `AGENTS.md` ("scripts never load `.env`").
3. **`create-booking-handler.test.ts:14-15`** — replace the real address/phone fixture with placeholder values (`[REDACTED_ADDRESS]` / a fake phone).
...
1. **`test-db.ts`** — delete the hand-written `DDL` array (lines 9–87). `openTestDb` reads `schema.sql` and applies it via the `statementsFromSql` splitter (move from `apply-schema.ts` to a shared module or duplicate locally). Tests then exercise the six objects the DDL copy was missing: `availability_schedule_idx`, `event_type_owner_slug_unique`, `event_type_hosts_event_type_idx`, `bookings_host_time_idx`, `host_occupancy_ticks_booking_idx`, `booking_host_unique`, plus the `credentials` table — including the real `event_type_owner_slug_unique` rejection of duplicate `(owner, slug)` pairs.
2. **Drift guard** — scheduled/nightly, non-blocking job: introspect live `sqlite_master`, assert every table/index name in `schema.ts` is present. Not a PR gate (see Item 0.5 trust boundary).
3. **`AGENTS.md`** — update "Schema lives in three places" to: two sources (`schema.ts` canonical, `schema.sql` applied identically to tests and prod), drift check enforces parity.
...
2. **Remove `host_mutexes`** — only after 2.1 passes. Delete `acquireHostMutex` (`create-booking-handler.ts:238-246`), its call (`:387`), and the `hostMutexes` table from `schema.ts`/`schema.sql`. Document the real serialization story in `AGENTS.md`: single-writer commit lock + `host_occupancy_tick_unique` + `SQLITE_BUSY` retry (`:322-344`).
...
2. **Schema** — add nullable `cancelled_at` to `bookings` in `schema.ts` + `schema.sql`.
...
2. Delete `HOST_FREE_RANGE_PAD_MINUTES = 1440` (`create-booking-handler.ts:82`) and the day-padding hack in `isHostFree` (`:188-193`) as part of this.
...
Single addition to `availability-engine.test.ts`: a working window with **both ends nonexistent** on the same local day (`02:15–02:45` on 2026-03-08, America/New_York) — assert it resolves to zero slots via the `firstValidAtOrAfter`/`lastValidAtOrBefore` fallback producing `end <= start`. Everything else already exists (`:41`, `:87`, `:190`, `:206`).
